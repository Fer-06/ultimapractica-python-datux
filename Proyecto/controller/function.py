...
def IngestDataProducts(app:App):
    ...
    dataCliente=GetDataSourceCliente()
    createTableCliente(conn)
    InsertDataCliente(bd,dataCliente)


def GetDataSourcePais():
    pathData="/workspaces/ultimapractica-python-datux/Proyecto/files"
...
def GetDataSourceCliente():
    pathData="/workspaces/ultimapractica-python-datux/Proyecto/files"
    df=pd.read_excel(pathData,sheet_name="Customer")
    df_customer=df[['Customer ID','Customer Name']].dropna().drop_duplicates()
    customer_tuples=[tuple(x) for x in df_customer.to_records(index=False)]
    return customer_tuples

def createTableCliente(conn:Connection):
    customer=Clientes()
    customer.create_table(conn)

def InsertDataCliente(bd:Database,data):
    bd.insert_many('CLIENTE',['name','customer ID'],data)
